...
parser.add_argument('--func', type=str, default="extract-mat-face-recon-uv-tex")  # 执行的功能模块列表
...
face_code_root = os.path.join(code_root, "face_mask")  # 人脸检测模块
...
mask_save_root = os.path.join(opt.save_root, "wholebody_mask")  # 掩码保存路径
...
if "uv" in opt.func:
    m_start = datetime.datetime.now()
    os.chdir(uv_code_root)
    os.system("python run_uv_pipeline.py --data_root %s" % (opt.save_root))
    os.chdir(code_root)
    
    m_end = datetime.datetime.now()
    write_log(f"[Module: uv] runtime: {m_end - m_start}")

# ========== 纹理生成模块 ==========
if "tex" in opt.func:
    m_start = datetime.datetime.now()
    os.chdir(tex_code_root)
    tex_data_root = os.path.join(opt.save_root, "texture_dataset")
    os.system("python run_texture.py --data_root %s" % tex_data_root)
    os.chdir(code_root)
